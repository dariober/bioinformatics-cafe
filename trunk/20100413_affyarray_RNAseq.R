...
# Microarray
#
# This script replaces 20100413_affyarray_RNAseq_old.R where replicate arrays 
# were first averaged then ln-transformed.
# Here, first log2 transform, then average.
# Note also that the input table is different (this is the original dataset as)
# given by Ronan/Lynsey). Probe names edited to match the original affymetrix.
#
...
## Input should look like this (tab-separated):
## --------------------------------------------------------------
...
limma_affy<- cbind(limma_affy[,1],              
    ...
    rowMeans(limma_affy[, c(21, 25)])   ## Note: Array in column 25 "s024_LWL5_24h_repl_2" should be excluded (see LabBook 09/07/2010)
    )
...
savePlot('M:/Documents/LabBook/LabBook_Figures/20100413_affyarray_RNAseq_DE.bmp', 'bmp')

length(de_table$adj.P.Val[de_table$adj.P.Val<0.01 & de_table$logFC > 0])
length(de_table$adj.P.Val[de_table$adj.P.Val<0.01 & de_table$logFC < 0])
  
# Write out to file
write.table(de_table, 'C:/data/lwl_bmdm_lps_avg_ct_0h_7h.limma', sep='\t', row.names=F)
...
affy_rnaseq<- read.table('F:/data/20100413/affy_rnaseq_db.txt', sep='\t', header=T)
dim(affy_rnaseq)
