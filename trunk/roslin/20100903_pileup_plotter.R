...
ensembl_gtf<- sqlQuery(conn_db, paste("
    ...
    where rname = '4' and 
          ...
          f_end <= ", right_pos, " and
          feature like 'exon'
    order by gene_id, f_start;
  "))
ensembl_gtf[1:50,c(1:5, 10, 11)]
## Genes start and end
gene<- data.frame(gene_id= unique(as.character(ensembl_gtf$gene_id)),
             gene_start= aggregate(ensembl_gtf$f_start, by=list(ensembl_gtf$gene_id), min)$x,
             gene_end= aggregate(ensembl_gtf$f_start, by=list(ensembl_gtf$gene_id), max)$x
             )
...
# ----------------------[ Large window ]------------------------------
  ...
  plot(x= pileup_chr4_lps$pos, y= pileup_chr4_lps$no_reads, type= 'n',
    xlab= '', ylab= 'Coverage', main= '',xlim=largew, ylim= c(0, max(pileup_chr4_lps$no_reads, pileup_chr4_ctrl$no_reads)), xaxt='n');
  ...
  points(x= pileup_chr4_ctrl$pos+1000, y= pileup_chr4_ctrl$no_reads, type= 'h', lwd= 1, col= 'blue')
 
  # Show where transcripts are. Different colour = different transcript
  cols<- ifelse(as.numeric(ensembl_gtf$gene_id) %% 2 == 0, 'firebrick4', 'darkgoldenrod1')
  rect(xleft= ensembl_gtf$f_start, xright= ensembl_gtf$f_end, ybottom= par("usr")[4]-200, ytop= par("usr")[4]-50, col= cols, border= cols)
  cols_seg<- ifelse(as.numeric(gene$gene_id) %% 2 == 0, 'firebrick4', 'darkgoldenrod1')
  segments(x0= gene$gene_start, x1= gene$gene_end, y0= par("usr")[4]-125, col= cols_seg, lwd= 2.5)
...
xv<- c(1,1,1,2,2,2,3,3,3)
cols<- vector(length=0)
for(i in 1:(length(xv))){
    x<- ifelse(xv[i] == xv[i+1], 'firebrick4', 'yellow')
    cols[i]<- x
    }
...
a<- c(1, 10, 20 ,30, 2)
b<- combn(a, 2)
babs<- apply(b, 2, function (x) abs(x[1]-x[2]))
b[,which(babs==min(babs))]
