"""
  Extract the coverage at each locus, but only where coverage is > n.
  Input file is a pileup generated by:
      samtools pileup -s -c -f

  Note: On eddie it is necessary to add module python 2.6 with (at bash shell):
...
dataset_id= '20100426_RNAseq_conc'
...
for line in pileup:
    line= line.split('\t')
    if int(line[7]) < 6:
        " Filter out positions covered by less the these many reads "
        continue
    if int(line[4]) < 20 or int(line[5]) <= -1:
...
#        continue
    ...
    counter += 1
    if counter % 250000 == 0:
        print(str(counter) + ' positions sequenced')
#    if counter >1000:
#        break
print('Total number of genome positions accepted: ' + str(counter))
pileup.close()
pileup_out.close()