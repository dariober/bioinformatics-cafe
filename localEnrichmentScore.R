...
docstring<- "Compute log fold change for enrichement and associated -log10(pvalue) for input files generated by localEnrichmentBed.py."
...
if(xargs$method %in% c('binomial', 'betabin')){
    ...
    for(i in 1:length(controls)){
        ...
        for(j in 1:nrow(target)){
            ...
            log2fc[j]<- (m[1,1] / m[2,1]) / (m[1,2] / m[2,2])
        }
    ...
    rankProdDT<- enrich[, list(RP= gmean(ranked), comb_p= -log10(Stouffer.test(pval)), avgLog2fc= mean(log2fc)), by= targetID]
