...
#
# Filter applied: Tolerate one base quality below a given threshold. If a second 
# low-quality base is encouneterd (reading the reads 5' -> 3'), chop away the
# read from the 2nd low quality base onward.

## --------------------------------[ User's input ]----------------------------

pair_end_file_1 = '/exports/work/vet_roslin_nextgen/dario/fastq/20091217_RNAseq_AM_LPS_CTRL/s_8_1_sequence.txt' ## Raw fastq files
pair_end_file_2 = '/exports/work/vet_roslin_nextgen/dario/fastq/20091217_RNAseq_AM_LPS_CTRL/s_8_2_sequence.txt'
...
def trim_sequence(seq_quality, min_trim_q):
    ...
    for i in range(0, len(seq_quality)):
        ...
        if low_q == 2:   ## Change int here to trim after third, forth etc. drop
